function varargout = cmidaq_update(varargin)
...
% Last Modified by GUIDE v2.5 14-Apr-2016 12:06:00

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @cmidaq_update_OpeningFcn, ...
    'gui_OutputFcn',  @cmidaq_update_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function cmidaq_update_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = cmidaq_update_OutputFcn(hObject, eventdata, handles)
                            ...
                            for j = 1:nofrepes
                                ...
                                if isChannelA == 1;
                                    if ismember(sprintf('trace_%02d_%04d',k-1,j-1),cellstr(char(infoA.Datasets.Name)))
                                        ...
                                        attA(record) = h5readatt(file,strcat(info.Groups.Groups.Groups(1,1).Name,'/', sprintf('scanpoint_%02d/trace_%02d_%04d',i-1,k-1,j-1)),'timestring');
                                    else
                                        if record ~= 1
...
if dimension < 4
    ...
    if length(c_info) ~= 4
        ...
        switch channel_show
            case 1
                gatedataraw = abs(mean(handles.current_dataA(:,backgroundmin:backgroundmax)')-mean(handles.current_dataA(:,gatemin:gatemax)'))*(gatemax-gatemin+1);
            case 2
                gatedataraw = abs(mean(handles.current_dataB(:,backgroundmin:backgroundmax)')-mean(handles.current_dataB(:,gatemin:gatemax)'))*(gatemax-gatemin+1);
            case 3
                gatedataraw = abs(mean(handles.current_dataC(:,backgroundmin:backgroundmax)')-mean(handles.current_dataC(:,gatemin:gatemax)'))*(gatemax-gatemin+1);
            case 4
                gatedataraw = abs(mean(handles.current_dataD(:,backgroundmin:backgroundmax)')-mean(handles.current_dataD(:,gatemin:gatemax)'))*(gatemax-gatemin+1);
        end
        ...
        ylabel('Parent ion yields (arb.units)','FontSize',14)%Intensiy (a.u.)','FontSize',14)
        ...
        if nofwaveforms == 1
            ...
            axis tight;
            ylabel('Parent ion yields (arb.units)','FontSize',14)
