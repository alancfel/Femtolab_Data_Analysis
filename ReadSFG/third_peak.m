function [eva,evag1,rad,radg1,wid,widg1] = third_peak(dat,video,fig)
...
for i=615:630
    ...
    x1=((pks(((abs(pks(:,2)-444))==min(abs(pks(:,2)-444))),2)-round(1.5*pks(((abs(pks(:,2)-444))==min(abs(pks(:,2)-444))),3))):(pks(((abs(pks(:,2)-444))==min(abs(pks(:,2)-444))),2)+round(1.5*pks(((abs(pks(:,2)-444))==min(abs(pks(:,2)-444))),3))));
    [FitResults,FitError] = peakfit([x1;dat(580,x1,i-614)],612,0,1,0,0,0,0,3);
    if video == 1
        title(sprintf('Ring No.3 -- %d mA',i),'Fontsize',14);
        frame = getframe(gcf);
        writeVideo(v,frame);
    end
    x2=((pks(((abs(pks(:,2)-801))==min(abs(pks(:,2)-801))),2)-round(1.5*pks(((abs(pks(:,2)-801))==min(abs(pks(:,2)-801))),3))):(pks(((abs(pks(:,2)-801))==min(abs(pks(:,2)-801))),2)+round(1.5*pks(((abs(pks(:,2)-801))==min(abs(pks(:,2)-801))),3))));
    ...
    rad(i-614,:) = abs(pks(((abs(pks(:,2)-444))==min(abs(pks(:,2)-444))),2)-pks(((abs(pks(:,2)-801))==min(abs(pks(:,2)-801))),2));
    wid(i-614,:) = mean([pks(((abs(pks(:,2)-444))==min(abs(pks(:,2)-444))),3),pks(((abs(pks(:,2)-801))==min(abs(pks(:,2)-801))),3)]);
end
close(gcf);
if video == 1
    close(v);
...
if fig == 1
    ...
    set(lh1,'fontsize',20);
    set(lh1,'box','off');
    title('Ring No.3 analysis','Fontsize',20);
