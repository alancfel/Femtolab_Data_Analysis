function [eva,evag1,rad,radg1,wid,widg1] = first_peak(dat,video,fig)
...
%     figure;findpeaks(dat(580,:,i-614),x,'MinPeakDistance',100,'Annotate','extents');
    ...
    if video == 1
        title(sprintf('Ring No.1 -- %d mA',i),'Fontsize',14);
        frame = getframe(gcf);
        writeVideo(v,frame);
    end     
...
if fig == 1
    ...
    set(lh1,'fontsize',20);
    set(lh1,'box','off');
    title('Ring No.1 analysis','Fontsize',20);
