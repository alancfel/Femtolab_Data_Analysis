function [eva,evag1,rad,radg1,wid,widg1] = fifth_peak(dat,video,fig)
...
for i=615:630
    ...
    %     figure;findpeaks(dat(580,:,i-614),x,'MinPeakDistance',20,'MinPeakWidth',6,'Annotate','extents');
    [pks(:,1),pks(:,2),pks(:,3),pks(:,4)] = findpeaks(dat(580,:,i-614),x,'MinPeakDistance',20,'MinPeakWidth',6);
    x1=((pks(((abs(pks(:,2)-350))==min(abs(pks(:,2)-350))),2)-round(1.5*pks(((abs(pks(:,2)-350))==min(abs(pks(:,2)-350))),3))):(pks(((abs(pks(:,2)-350))==min(abs(pks(:,2)-350))),2)+round(1.5*pks(((abs(pks(:,2)-350))==min(abs(pks(:,2)-350))),3))));
    [FitResults,FitError] = peakfit([x1;dat(580,x1,i-614)],612,0,1,0,0,0,0,3);
    if video == 1
        title(sprintf('Ring No.5 -- %d mA',i),'Fontsize',14);
        frame = getframe(gcf);
        writeVideo(v,frame);
    end
    x2=((pks(((abs(pks(:,2)-900))==min(abs(pks(:,2)-900))),2)-round(1.5*pks(((abs(pks(:,2)-900))==min(abs(pks(:,2)-900))),3))):(pks(((abs(pks(:,2)-900))==min(abs(pks(:,2)-900))),2)+round(1.5*pks(((abs(pks(:,2)-900))==min(abs(pks(:,2)-900))),3))));
    ...
    rad(i-614,:) = abs(pks(((abs(pks(:,2)-350))==min(abs(pks(:,2)-350))),2)-pks(((abs(pks(:,2)-900))==min(abs(pks(:,2)-900))),2));
    wid(i-614,:) = mean([pks(((abs(pks(:,2)-350))==min(abs(pks(:,2)-350))),3),pks(((abs(pks(:,2)-900))==min(abs(pks(:,2)-900))),3)]);
end
close(gcf);
if video == 1
    close(v);
...
if fig == 1
    ...
    set(lh1,'fontsize',20);
    set(lh1,'box','off');
    title('Ring No.5 analysis','Fontsize',20);
