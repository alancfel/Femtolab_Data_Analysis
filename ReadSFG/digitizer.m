function varargout = digitizer(varargin)
...
% Last Modified by GUIDE v2.5 05-Apr-2018 18:47:31

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @digitizer_OpeningFcn, ...
    'gui_OutputFcn',  @digitizer_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function digitizer_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = digitizer_OutputFcn(hObject, eventdata, handles)
...
global machename;
global step;
[filename, pathname] = uigetfile({'*.hdf5';'*.h5';'*.mat'}, 'Open Data');
if filename ~=0
    ...
    if strcmp(filename(dot+1:end), 'mat')
        ...
        show_Callback(handles.show, eventdata, handles)
    else if strcmp(filename(dot+1:end), 'hdf5') || strcmp(filename(dot+1:end), 'h5')
            info = h5info(file);
            if isfield(info.Groups,'Attributes') && length(info.Groups(1).Attributes)==7
                if isfield(info.Groups(1).Attributes,'Value')
                    samplerate = info.Groups(1).Attributes(6).Value;
                    if samplerate == 2e+09
                        string_list{1} = 'Software';
                        string_list{2} = 'External';
                        string_list{3} = 'Level';
                        string_list{4} = 'Internal';
%                         settings = h5read(file,strcat('/',info.Groups(1).Datasets.Name));
                        recordsize = double(info.Groups(1).Datasets(1).Dataspace.Size(2));%info.Groups(1).Attributes(5).Value; %Samples per record
                        nofrecords = length(info.Groups); % measurement number = step number * cycle number
                        nofrepes = info.Groups(1).Attributes(2).Value; % software average number
                        nofwaveforms = info.Groups(1).Attributes(3).Value; % hardware average number
                        selected_trigger = info.Groups(1).Attributes(4).Value; % find(strcmp(string_list,info.Groups(1).Attributes(6).Value));
                        machename = char(info.Groups(1).Attributes((strmatch('Settings',char(info.Groups(1).Attributes.Name)))).Value(1));
                        stepvalue = [];
                        for att = 1:nofrecords
                            if strcmp(machename,'DDG')
                                stepvalue = [stepvalue,str2double(char(info.Groups(att).Attributes((strmatch('Settings',char(info.Groups(att).Attributes.Name)))).Value(2)))*1e6];
                            else if strcmp(machename,'vertical')
                                    stepvalue = [stepvalue,str2double(char(info.Groups(att).Attributes((strmatch('Settings',char(info.Groups(att).Attributes.Name)))).Value(2)))/800];
                                else
                        ...
                        for i = 1:nofrecords
                            for j = 1:nofrepes
                                record = record+1;
                                if isChannelA == 1;
                                    dataA = h5read(file,strcat('/',num2str(i),'/Channel A'));
                                    handles.current_dataA(record,:) = dataA(j,:);
                                end
                                if isChannelB == 1;
                                ...
                                if isChannelC == 1;
                                    dataC = h5read(file,strcat('/',num2str(i),'/Channel C'));
                                    handles.current_dataC(record,:) = dataC(j,:);
                                end
                                if isChannelD == 1;
...
if dimension == 1
    ...
    %         gatemax = n(m == gate(4));
    % end
    gatedata = abs(mean(handles.current_dataD(:,backgroundmin:backgroundmax)')-mean(handles.current_dataD(:,gatemin:gatemax)'))*(gatemax-gatemin+1);
    gatemean = abs(mean(handles.current_dataD(:,backgroundmin:backgroundmax)')-mean(handles.current_dataD(:,gatemin:gatemax)'));
    for j=1:nofrecords
    ...
    plot(gatedata,'-o','linewidth',2,'markersize',8);
    ylabel('Intensiy (a.u.)','FontSize',14)
    xlabel('Measurement series','FontSize',14)
    set(gca,'fontsize',20);
