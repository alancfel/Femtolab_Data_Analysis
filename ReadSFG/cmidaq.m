function varargout = cmidaq(varargin)
...
% Last Modified by GUIDE v2.5 08-Dec-2015 11:49:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @cmidaq_OpeningFcn, ...
    'gui_OutputFcn',  @cmidaq_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function cmidaq_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = cmidaq_OutputFcn(hObject, eventdata, handles)
...
switch dimension
    ...
    case 3
        ...
        if ylabelmod == 1;
            h(1) = plot(x,mean(handles.current_dataA(((sliderVal-1)*(nofrepes*nofcycle)+1):(sliderVal*(nofrepes*nofcycle)),:),1),'r','DisplayName','ChannelA','visible',legA);
        else
    ...
    case 7
        ...
        switch channel_show
            ...
            case 2
                ...
                %                         newdata = data;
                %                     end
                h(2) = pcolor(x,ylabel2,data);shading interp;
            case 3
                data = [];
                for index = 1:length(step)
                ...
                if samplerate == 2e+09
                    ...
                    if strcmp(machename,'Channel.A')
                        ...
                        if length(stepvalue) == nofrecords/nofcycle
                            step = stepvalue;
                        else if length(stepvalue) > nofrecords/nofcycle
                    ...
                    for i = 1:nofrecords/nofcycle; %[1:1044,1047:1263,1265:nofrecords]
                        infoA = h5info(file,strcat(info.Groups.Groups.Groups(2,1).Name,'/', sprintf('scanpoint_%d/',i-1)));
                        infoC = h5info(file,strcat(info.Groups.Groups.Groups(1,1).Name,'/', sprintf('scanpoint_%d/',i-1)));
                        for k = 1:nofcycle
                            for j = 1:nofrepes
                                record = record+1;%i;%record+1;
                                if isChannelA == 1;
                                    if ismember(sprintf('trace_%d_%d',k-1,j-1),cellstr(char(infoA.Datasets.Name)))
                                        dataA = h5read(file,strcat(info.Groups.Groups.Groups(2,1).Name,'/', sprintf('scanpoint_%d/trace_%d_%d',i-1,k-1,j-1)))';%1_%d',record-1)))'; %strcat('/',num2str(i),'/Channel A'));
                                        handles.current_dataA(record,:) = single(dataA(1,:)/nofwaveforms);
                                        attA(record) = h5readatt(file,strcat(info.Groups.Groups.Groups(2,1).Name,'/', sprintf('scanpoint_%d/trace_%d_%d',i-1,k-1,j-1)),'timestamp');
                                    else
                                        if record ~= 1
                                            handles.current_dataA(record,:) = handles.current_dataA(record-1,:); %dataA(1,:);
                                            attA(record) = attA(record-1);
                                            fprintf('Channel A scanpoint_%d/trace_%d_%d is not exist in file\n',i-1,k-1,j-1);%set(handles.textStatus, 'String', sprintf('trace_%d_%d is not exist in file',k-1,j-1))
                                        else
                                            attA(record) = 0;
                                            fprintf('Channel A scanpoint_%d/Attribute trace_%d_%d is not exist in file\n',i-1,k-1,j-1);
                                        end
                                    end
                                end
                                if isChannelB == 1;
                                ...
                                if isChannelC == 1;
                                    if ismember(sprintf('trace_%d_%d',k-1,j-1),cellstr(char(infoC.Datasets.Name)))
                                        dataC = h5read(file,strcat(info.Groups.Groups.Groups(1,1).Name,'/', sprintf('scanpoint_%d/trace_%d_%d',i-1,k-1,j-1)))'; %strcat('/',num2str(i),'/Channel C'));
                                        ...
                                        if record ~= 1
                                            ...
                                            fprintf('Channel C scanpoint_%d/trace_%d_%d is not exist in file\n',i-1,k-1,j-1);%set(handles.textStatus, 'String', sprintf('trace_%d_%d is not exist in file',k-1,j-1))
                                        else
                                            fprintf('Channel C scanpoint_%d/Attribute trace_%d_%d is not exist in file\n',i-1,k-1,j-1);
                                        end
...
if dimension < 4
    ...
    if length(c_info) ~= 4
        ...
        if nofcycle ~= 1
            ...
            stdsignal = std(reshape(msignalraw, nofrecords/nofcycle,[])',1,1);
        else
            msignal = msignalraw;
        ...
        num_lines = 1;
        defaultans = {'20'};
        gate = inputdlg(prompt,dlg_title,num_lines,defaultans);
...
%         saveas(gcf,strcat(figurename,'_',sprintf('gate_%d',mgate),'_ave','.png'));
        ...
        if nofwaveforms == 1
            ...
            shot=reshape(laser_shot,50,[]);
            gatemean50 = mean(reshape(gatedata, 50,[]),1)/61.04;
            h9 = figure('PaperSize',[8.267716 15.692913]);
            bar(shot(50,:),gatemean50);
            axis tight;
            ylabel(sprintf('Ion yields at %d Da (arb.units)',mgate),'FontSize',14);%Intensiy (a.u.)','FontSize',14)
...
%             saveas(gcf,strcat(figurename,'_',sprintf('gate_%d',mgate),'_all','.png'));
            ...
            errorbar(xlabel2,msignal/61.04,stdsignal/61.04,'linewidth',2);
            ylabel(sprintf('Ion yields at %d Da (arb.units)',mgate),'FontSize',14);%Intensiy (a.u.)','FontSize',14)
...
%                 eval(sprintf('%s_gate%d_err = stdcsignalraw/61.04; ', figurename, mgate));
            ...
            errorbar(xlabel2,csignalraw/61.04,stdcsignalraw/61.04,'linewidth',2);
            ylabel(sprintf('Ion yields at %d Da (arb.units)',mgate),'FontSize',14);%Intensiy (a.u.)','FontSize',14)
            xlabel(xname,'FontSize',14)
            set(gca,'fontsize',20);
            ...
            plot(xlabel2,reshape(msignalraw, nofrecords/nofcycle,[])'/61.04,'-o','linewidth',2,'markersize',8);
        ...
        if nofcycle ~= 1
            ...
            errorbar(repmat(xlabel2,nofcycle,1)',reshape(msignalraw, nofrecords/nofcycle,[])/61.04,reshape(stdsignalraw, nofrecords/nofcycle,[])/61.04,'linewidth',2);
            ylabel('Ion Numbers per laser shot','FontSize',14)
            xlabel(xname,'FontSize',14)
            set(gca,'fontsize',20);
